	...
	private class ConfirmListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if("".equals(DBUtil.user)||"".equals(DBUtil.pwd)){
				Notice.display(Modify.this.getContentPane(), "DB账号缺失！");
			}
			else if("".equals(user)||"".equals(pwd)){
				Notice.display(Modify.this.getContentPane(),"账号或密码不能为空！");
			}
			else if("".equals(DoWhatAndSendMes.udao.getUser(user).getAccount())){
				Notice.display(Modify.this.getContentPane(), "用户不存在！");
			}
			else{
				User u = new User();
				u.setAccount(user);
				...
				Notice.display(Modify.this.getContentPane(), "密码修改成功！");
				Modify.this.dispose();
			}
		}
	}
